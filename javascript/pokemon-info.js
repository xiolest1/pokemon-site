/*******************************************
 * 1) TCG Card Detail View (ADDED)
 ********************************************/
async function openTCGCardInfo(cardId) {
    // Hide the default "no Pokémon selected" message
    document.getElementById('current-pokemon-empty').classList.add('hide');
  
    // On wide screens, animate the panel out first
  ...
  async function fetchTCGCardInfo(cardId) {
    try {
      ...
      // Now slide in the info panel
      slideInPokemonInfo();
  
      // On small screens, open the responsive overlay
      if (window.innerWidth < 1100) {
  ...
  function setupTCGCardUI(card) {
    // 1) Hide the Pokémon info container & show TCG container
    ...
    // 2) Update the "top" image with the TCG card's large image
    //    (We'll pick 'images.large' if available, else 'images.small')
    ...
    imageEl.style.height = 'auto'; // or some fixed size if you prefer
  
    // 3) Fill TCG fields
    document.getElementById('current-tcg-id').innerText = `Card ID: ${card.id}`;
    document.getElementById('current-tcg-name').innerText = dressUpPayloadValue(card.name);
    document.getElementById('current-tcg-set').innerText = card.set?.name || 'Unknown Set';
  
    // If it's a Pokémon card, it might have "types" or "subtypes"
    let typeInfo = (card.types && card.types.join(', ')) || card.supertype;
    let subtypes = (card.subtypes && card.subtypes.join(', ')) || '';
    if (subtypes) {
      typeInfo += ` / ${subtypes}`;
    }
    document.getElementById('current-tcg-type').innerText = typeInfo || 'No type';
  
    // HP (some trainer cards might not have HP)
    document.getElementById('current-tcg-hp').innerText = card.hp ? `${card.hp} HP` : '—';
  
    // Rules (often an array of strings)
    if (card.rules && card.rules.length > 0) {
      document.getElementById('current-tcg-rules').innerText = card.rules.join(' ');
    } else {
      document.getElementById('current-tcg-rules').innerText = 'No rules text';
    }
  
    // 4) Because we only show TCG info, hide any leftover "Pokémon details" 
    //    that might remain. We did that above with .classList.add('hide').
  }
  
  /*******************************************
   * 2) Pokémon Info (Unchanged Original)
   ********************************************/
  function openInfo(id) {
    document.getElementById('current-pokemon-empty').classList.add('hide');
  
    // If showing TCG container, hide it now
  ...
  /**------------------------- Responsive + Animations (Unchanged) ---------------------------------------------------------------- */
